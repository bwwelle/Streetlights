...
(function ($) {
	...
	$.fn.makeEditable = function (options) {
        ...
        function fnOnRowEditing(event) {
			...
			if (properties.fnOnEditing()) {
				if (oEditRowForm.valid()) {
					iDisplayStart = fnGetDisplayStart();
					properties.fnStartProcessingMode();

					if (properties.bUseFormsPlugin) {
						//Still in beta(development)
						$(oEditRowForm).ajaxSubmit({
							dataType : 'xml',
							success : function (response, statusString, xhr) {
								if (xhr.responseText.toLowerCase().indexOf("error") != -1) {
									properties.fnEndProcessingMode();
									properties.fnShowError(xhr.responseText.replace("Error", ""), "edit");
									properties.fnOnEdited("failure");
								} else {
									fnOnRowEdited(xhr.responseText);
								}

							},
							error : function (response) {
								properties.fnEndProcessingMode();
								properties.fnShowError(response.responseText, "edit");
						...
						var params = oEditRowForm.serialize();
						$.ajax({
							'url' : properties.sEditURL,
							'data' : params,
							'type' : properties.sEditHttpMethod,
							'dataType' : properties.sEditDataType,
							success : fnOnRowEdited,
							error : function (response) {
								properties.fnEndProcessingMode();
								properties.fnShowError(response.responseText, "edit");
								properties.fnOnAdded("failure");
		...
		function _fnOnEditedPosted(data) {
			///<summary>Callback function called BEFORE an edited record is posted to the server</summary>
			///TODO: Check this

			return true;
		}

		function fnOnRowEdited(data) {
			///<summary>
			///Function that is called when a row is edited, and Ajax response is returned from server
			/// This function takes data from the edit form and adds them into the table.
			///</summary>
			///<param name="data" type="int">Id of the new row that is returned from the server</param>

			properties.fnEndProcessingMode();

			if (properties.fnOnEditedRowPosted(data)) {
				...
				/*}*/
				//Close the dialog
				oEditRowForm.dialog('close');
				$(oEditRowForm)[0].reset();
				$(".error", $(oEditRowForm)).html("");
		...
		function fnOnCancelRowEditing(event) {
			///<summary>
			///Event handler function that is executed when a user press cancel button in the edit row form
			///This function clean the edit form and error messages if some of them are shown
			///</summary>
			///<param name="event" type="int">DOM event that caused an error</param>

			//Clear the validation messages and reset form
			$(oEditRowForm).validate().resetForm(); // Clears the validation errors
			$(oEditRowForm)[0].reset();

			$(".error", $(oEditRowForm)).html("");
			$(".error", $(oEditRowForm)).hide(); // Hides the error element
		...
		return this.each(function () {
			...
			if (oEditRowForm.length != 0) {
				...
				var iColumnCount = oSettings.aoColumns.length;
				for (i = 0; i < iColumnCount; i++) {
					if ($("[rel=" + i + "]", oEditRowForm).length == 0)
						properties.fnShowError("In the form that is used for editing records cannot be found an input element with rel=" + i + " that will be bound to the value in the column " + i + ". See http://code.google.com/p/jquery-datatables-editable/wiki/AddingNewRecords#Add_new_record_form for more details", "init");
				}

				if (properties.oEditRowFormOptions != null) {
					properties.oEditRowFormOptions.autoOpen = false;
				} else {
					properties.oEditRowFormOptions = {
						autoOpen : false
					};
				}
				...
				if (oEditRowForm[0].nodeName.toLowerCase() == "form") {
					...
					$("form", oEditRowForm[0]).submit(function (event) {
						fnOnRowEditing(event);
						return false;
					});
				}

				// array to add default buttons to
				var aEditRowFormButtons = [];

				oConfirmRowEditingButton = $("#" + properties.sEditRowOkButtonId, oEditRowForm);
				if (oConfirmRowEditingButton.length == 0) {
					//If someone forgotten to set the button text
					if (properties.oEditRowOkButtonOptions.text == null
						 || properties.oEditRowOkButtonOptions.text == "") {
						properties.oEditRowOkButtonOptions.text = "Ok";
					}
					properties.oEditRowOkButtonOptions.click = fnOnRowEditing;
					properties.oEditRowOkButtonOptions.id = properties.sEditRowOkButtonId;
					// push the edit button onto the array
					aEditRowFormButtons.push(properties.oEditRowOkButtonOptions);
				} else {
					oConfirmRowEditingButton.click(fnOnRowEditing);
				}

				oCancelRowEditingButton = $("#" + properties.sEditRowCancelButtonId);
				if (oCancelRowEditingButton.length == 0) {
					//If someone forgotten to the button text
					if (properties.oEditRowCancelButtonOptions.text == null
						 || properties.oEditRowCancelButtonOptions.text == "") {
						properties.oEditRowCancelButtonOptions.text = "Cancel";
					}
					properties.oEditRowCancelButtonOptions.click = fnOnCancelRowEditing;
					properties.oEditRowCancelButtonOptions.id = properties.sEditRowCancelButtonId;
					// push the cancel button onto the array
					aEditRowFormButtons.push(properties.oEditRowCancelButtonOptions);
				} else {
					oCancelRowEditingButton.click(fnOnCancelRowEditing);
				}
				// if the array contains elements, add them to the dialog
				if (aEditRowFormButtons.length > 0) {
					oEditRowForm.dialog('option', 'buttons', aEditRowFormButtons);
				}
				//Issue: It cannot find it with this call:
				//oConfirmRowEditingButton = $("#" + properties.sEditRowOkButtonId, oEditRowForm);
				//oCancelRowEditingButton = $("#" + properties.sEditRowCancelButtonId, oEditRowForm);
				oConfirmRowEditingButton = $("#" + properties.sEditRowOkButtonId);
				oCancelRowEditingButton = $("#" + properties.sEditRowCancelButtonId);

				if (properties.oEditRowFormValidation != null) {
					oEditRowForm.validate(properties.oEditRowFormValidation);
				}
			} else {
				oEditRowForm = null;
			}
			...
			//If an add, delete, and edit buttons does not exists but Add-delete toolbar is specificed
			//Autogenerate these buttons
			oAddDeleteEditToolbar = $(properties.sAddDeleteEditToolbarSelector);
			if (oAddDeleteEditToolbar.length != 0) {
				if (oAddNewRowButton == null && properties.sAddNewRowButtonId != ""
					 && oAddNewRowForm != null) {
					oAddDeleteEditToolbar.append("<button id='" + properties.sAddNewRowButtonId + "' class='add_row'>Add</button>");
					oAddNewRowButton = $("#" + properties.sAddNewRowButtonId);
					oAddNewRowButton.click(function () {
						oAddNewRowForm.dialog('open');
					});
			...
			//If form cancel button exists convert it to the JQuery-ui button
			if (oCancelRowEditingButton != null) {
				if (properties.oEditRowCancelButtonOptions != null) {
					oCancelRowEditingButton.button(properties.oEditRowCancelButtonOptions);
				}
			}
			...
			if (!properties.bUseKeyTable) {
				...
				//Enable delete/edit button if row is selected, disable delete/edit button if selected class is removed
